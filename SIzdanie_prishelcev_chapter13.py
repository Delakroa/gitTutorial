...
# Метод spritecollideany() получает два аргумента: спрайт и группу. Метод пыта-
...
# В данном случае он перебирает группу aliens и возвращает первого пришельца,
# столкнувшегося с кораблем.
# Если ни одна коллизия не обнаружена, spritecollideany() возвращает None, и блок
# if в точке  не выполняется. Если же будет обнаружен пришелец, столкнувшийся
# с кораблем, метод возвращает этого пришельца, и выполняется блок if: выводится
# сообщение Ship hit!!! . (При столкновении пришельца с кораблем необходимо
...
# работает правильно. Команда print всего лишь позволяет легко проверить правиль-
...
# минальном окне появляется сообщение Ship hit!!!. В ходе тестирования этого
# аспекта присвойте alien_drop_speed более высокое значение (например, 50 или
...
# с кораблем. Вместо того чтобы уничтожать экземпляр ship и создавать новый,
...
# счета очков.)
# Напишем новый класс GameStats для ведения статистики и сохраним его в файле
# game_stats.py:
...
# На все время работы Alien Invasion будет создаваться один экземпляр GameStats,
# но часть статистики должна сбрасываться в начале каждой новой игры. Для этого
# бульшая часть статистики будет инициализироваться в методе reset_stats()
# вместо __init__(). Этот метод будет вызываться из __init__(), чтобы статистика
# правильно инициализировалась при первом создании экземпляра GameStats ,
# а метод reset_stats() будет вызываться в начале каждой новой игры.
# Пока в игре используется всего один вид статистики — значение ships_left, из-
# меняющееся в ходе игры. Количество кораблей в начале игры хранится в settings.py
# под именем ship_limit:
...
# Также необходимо внести ряд изменений в alien_invasion.py для создания экзем-
# пляра GameStats:
...
# Мы импортируем новый класс GameStats , создаем экземпляр stats , а затем
# добавляем аргументы stats, screen и ship в вызов update_aliens() . Эти аргу-
...
# Сначала мы импортируем функцию sleep() из модуля time, чтобы приостановить
# игру . Новая функция ship_hit() управляет реакцией игры на столкновение
# корабля
# с пришельцем. Внутри ship_hit() число оставшихся кораблей уменьша-
# ется на 1 , после чего происходит очистка групп aliens и bullets .
# Затем программа создает новый флот и выравнивает корабль по центру нижнего
# края . (Вскоре мы добавим метод center_ship() в класс Ship.) Наконец, после
...
# ся с пришельцем . После завершения паузы sleep() код переходит к функции
# update_screen(), которая перерисовывает новый флот на экране.
# Также необходимо обновить определение update_aliens() и добавить параметры
# stats, screen и bullets , чтобы эти значения можно было передать при вызове
# ship_hit().
...
# Чтобы выровнять корабль по центру, мы задаем атрибуту center корабля значение,
# соответствующее центру экрана (полученное при помощи атрибута screen_rect).
...
# новую функцию, которая будет называться update_aliens():
...
# Функция check_aliens_bottom() проверяет, есть ли хотя бы один пришелец, до-
# бравшийся до нижнего края экрана. Условие выполняется, когда атрибут rect.
# bottom пришельца больше или равен атрибуту rect.bottom экрана . Если при-
# шелец добрался до низа, вызывается функция ship_hit(). Если хотя бы один
# пришелец добрался до нижнего края, проверять остальных уже не нужно, поэтому
# после вызова ship_hit() цикл прерывается.
# Функция check_aliens_bottom() вызывается после обновления позиций всех
...
# чение ships_left просто продолжает уходить в отрицательную бесконечность.
# Добавим в GameStats новый атрибут — флаг game_active, который завершает игру
...
# Добавим в ship_hit() код, который сбрасывает флаг game_active в состояние False
...
# Бульшая часть кода ship_hit() осталась неизменной. Весь существующий код
# был перемещен в блок if, который проверяет, что у игрока остался хотя бы один
# корабль. Если корабли не кончились, программа создает новый флот, делает паузу
# и продолжает игру. Если же игрок потерял последний корабль, флаг game_active
# переводится в состояние False.

# ---------------------------------------------------------------------------------------------------------------------

# Определение исполняемых частей игры

# В файле alien_invasion.py необходимо определить части игры, которые должны вы-
...
# В основном цикле всегда должна вызываться функция check_events(), даже если
...
# не активна,
# обновлять позиции игровых элементов не нужно.
