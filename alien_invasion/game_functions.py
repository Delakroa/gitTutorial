...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets, play_button):
    ...
    for bullet in bullets.sprites():
        bullet.draw_bullet()
    ship.blitme()
    aliens.draw(screen)  # Когда вы вызываете метод draw() для группы, Pygame автоматически выводит каждый
    # элемент группы в позиции, определяемой его атрибутом rect
...
def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets)  # не понятно нужно ли это
...
после чего обновляет позиции всех пришельцев во флоте."""
    ...
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets)
        # print("Корабль сбит !!!") # для проверки анонса попадания
