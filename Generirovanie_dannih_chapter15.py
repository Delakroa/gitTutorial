...
# шим размером (s=100) по сравнению с остальными точками. Для выделения конеч-
# ной точки последняя пара координат x и y выводится с размером 100. Обязательно
# вставьте этот код непосредственно перед вызовом plt.show(), чтобы начальная
...
# Функция plt.axes()  переводит флаг видимости каждой оси в состояние False.
...
# мы увеличим значение num_points при создании экземпляра RandomWalk и отрегу-
...
# степени соответствует реальным данным), и каждая точка рисуется с размером s=1.
...
# Или, если вы знаете разрешение экрана в вашей системе, передайте его figure()
# в параметре dpi для выбора размера, эффективно использующего доступное про-
# странство:
#
...
# Установка Pygal
#
# Установите Pygal при помощи pip. (Если вы еще не использовали pip, обратитесь
# к разделу «Установка пакетов Python с использованием pip» на с. 227.)
...
# Метод __init__() получает один необязательный аргумент. Если при создании
...
# D8 и т. д.)
#
# Метод roll() использует функцию randint() для получения случайного числа
# в диапазоне от 1 до количества граней . Функция может вернуть начальное зна-
# чение , конечное значение (num_sides) или любое целое число в этом диапазоне.
...
# В точке  создается экземпляр Die с шестью гранями по умолчанию. В точке 
# моделируются 100 бросков кубика, а результат каждого броска сохраняется в спи-
# ске results. Выборка выглядит примерно так:
...
# Беглое знакомство с результатами показывает, что класс Die работает. В результа-
...
# список frequencies, в котором хранится количество выпадений каждого значения.
...
# присоединяет полученное значение к списку frequencies . Содержимое списка
...
# Чтобы построить столбцовую диаграмму, мы создаем экземпляр pygal.Bar(), ко-
# торый сохраняется в переменной hist . Затем мы задаем атрибут title объекта
# hist (обычная строка, используемая как заголовок гистограммы), используем воз-
# можные результаты броска D6 как метки оси x , а также добавляем надпись для
# каждой из осей. Метод add() используется для добавления на гистограмму серии
...
# SVG, что предполагает имя файла с расширением .svg.
...
# был сохранен файл die_visual.py). Диаграмма должна выглядеть примерно так, как
...
# ному с каждого кубика) и сохраняет сумму в results. Сохраните копию die_visual.
...
# Создав два экземпляра Die, мы бросаем кубики и вычисляем сумму для каждого
# броска . Самый большой возможный результат (12) вычисляется суммировани-
# ем наибольших результатов на обоих кубиках; мы сохраняем его в max_result .
...
# каждого значения от 2 до max_result . (Также можно было использовать диапазон
# range(2, 13), но он работал бы только для двух кубиков D6. При моделировании
...
# (Если бы список x_labels был намного длиннее, то его было бы удобнее сгенери-
...
# экземпляра Die1 и изменяем первый цикл для моделирования 50 000 бросков
# вместо 1000. Наименьший возможный результат, как и прежде, равен 2, зато наи-
# больший увеличился до 16; мы соответственно изменяем заголовок, метки оси x
# и метки серии данных .
# На рис. 15.13 показана полученная диаграмма. Вместо одного наиболее вероят-
